...
} from "../../modules/tokens/tokens.service";
...
export async function processAnimateJob(
  ...
  try {
    ...
    if (
      results.length > 0 &&
      results.every((r) => r.status === TRIPO_JOB_CONFIG.TRIPO_JOB_STATUSES.FAILED)
    ) {
      throw new Error("All animation retargets failed");
    }

    return { animations: results };
  } catch (err) {
    await refundTokenUsageByIdempotencyKey(tokenUsageIdempotencyKey).catch(() => {});
    throw err;
  }
}
