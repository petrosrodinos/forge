...
router.get("/", async (req, res, next) => {
  try {
    res.json(await figuresSvc.listFigures(req.userId));
  } catch (err) { next(err); }
});

router.get("/:id", async (req, res, next) => {
  try {
    const figure = await figuresSvc.getFigureById(req.userId, req.params.id);
...
router.put("/:id", async (req, res, next) => {
  try {
    const figure = await figuresSvc.updateFigure(req.userId, req.params.id, req.body);
    if (!figure) return res.status(404).json({ error: "Figure not found" });
    res.json(figure);
  } catch (err) { next(err); }
});
...
router.delete("/:id", async (req, res, next) => {
  try {
    const figure = await figuresSvc.deleteFigure(req.userId, req.params.id);
    if (!figure) return res.status(404).json({ error: "Figure not found" });
    res.json(figure);
  } catch (err) { next(err); }
});
...
router.post("/:id/generate-image", async (req, res, next) => {
  try {
    ...
    const result = await figuresSvc.generateAndSaveFigureImage(req.userId, {
