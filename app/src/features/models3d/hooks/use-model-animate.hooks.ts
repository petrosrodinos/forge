...
import type { AnimateResult } from "@/features/models3d/interfaces/model3d-jobs.interfaces";
import { notifyInsufficientTokensIf402 } from "@/store/insufficientTokensModalStore";
import { API_BASE_URL } from "@/utils/constants";

export function useModelAnimate(onComplete: (r: AnimateResult) => void) {
  ...
  async function run(model3dId: string, animations: string[]) {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE_URL}/api/models3d/${encodeURIComponent(model3dId)}/animate`, {
        method: "POST",
        credentials: "include",
